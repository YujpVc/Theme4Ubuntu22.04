...
class OpenWeatherMenuButton extends PanelMenu.Button {
    ...
    rebuildCurrentWeatherUi() {
        ...
        this._currentWeatherFeelsLike = new St.Label({
            text: '...',
            style_class: 'openweather-current-databox-value',
            x_expand: true,
        ...
        const numericCaptionOffset = this.locale === 'zh' ? -4 : 0;
        const windCaptionOffset = this.locale === 'zh' ? -1 : 0;
        const makeDetailRow = (caption, value, captionOffset = 0) => {
            let row = new St.BoxLayout({
                x_expand: true,
                style_class: 'openweather-current-databox-row'
            });
            let captionLabel = new St.Label({
                text: caption,
                style_class: 'openweather-current-databox-caption',
                x_expand: false,
                x_align: Clutter.ActorAlign.END,
                y_expand: true,
                y_align: Clutter.ActorAlign.CENTER,
                translation_y: captionOffset
            });
            captionLabel.set_height(28);
            value.set_height(28);
            row.add_actor(new St.Bin({
                child: captionLabel,
                x_expand: false,
                y_expand: true,
                y_align: Clutter.ActorAlign.CENTER
        ...
        makeDetailRow(_('Feels Like:'), this._currentWeatherFeelsLike, numericCaptionOffset);
        makeDetailRow(_('Humidity:'), this._currentWeatherHumidity, numericCaptionOffset);
        makeDetailRow(_('Pressure:'), this._currentWeatherPressure, numericCaptionOffset);
        makeDetailRow(_('Wind:'), this._currentWeatherWind, windCaptionOffset);
        makeDetailRow(_('Gusts:'), this._currentWeatherWindGusts, numericCaptionOffset);
